...
try:
    from PaddleScience.examples.UTAE.src.backbones.convlstm import ConvLSTM, ConvLSTMCell
except ModuleNotFoundError:
    # 最小回退：把本地 backbones 路径加入 sys.path 后直接导入模块名
    BACKBONES_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "PaddleScience", "examples", "UTAE", "src", "backbones")
    if BACKBONES_DIR not in sys.path:
        sys.path.append(BACKBONES_DIR)
    from convlstm import ConvLSTM, ConvLSTMCell

# 修补 ConvLSTMCell.forward：确保将 4*hidden_dim 通道按 4 等分，而不是按 hidden_dim 等分
# 避免出现 'expected 4, got 2' 的 unpack 错误（paddle.split 的第一个参数是切块数量）

def _patched_convlstmcell_forward(self, input_tensor, cur_state):
    h_cur, c_cur = cur_state
    combined = paddle.concat([input_tensor, h_cur], axis=1)
    combined_conv = self.conv(combined)
    cc_i, cc_f, cc_o, cc_g = paddle.split(combined_conv, num_or_sections=4, axis=1)
    i = paddle.nn.functional.sigmoid(cc_i)
    f = paddle.nn.functional.sigmoid(cc_f)
    o = paddle.nn.functional.sigmoid(cc_o)
    g = paddle.tanh(cc_g)
    c_next = f * c_cur + i * g
    h_next = o * paddle.tanh(c_next)
    return h_next, c_next

# 应用补丁（若类存在）
try:
    setattr(ConvLSTMCell, "forward", _patched_convlstmcell_forward)
except Exception:
    pass
...
class CLSTM_UNet(paddle.nn.Layer):
    def __init__(self, Par, unet_dim=16, dim_mults=(1, 2, 4, 8)):
        super(CLSTM_UNet, self).__init__()
        self.Par = Par
        # ConvLSTM: 5D 输入 [B, T, C, H, W]，输出最后一层的序列 [B, T, hidden, H, W]
        # 为了不改变 Unet2D 的输入通道数和其内部 reshape 逻辑，设置 hidden_dim = Par["nf"].
        self.clstm = ConvLSTM(
            input_size=(Par["nx"], Par["ny"]),
            input_dim=Par["nf"],
            hidden_dim=Par["nf"],
            kernel_size=(3, 3),
            num_layers=1,
        ...
        # Unet2D 保持原参数配置，内部会按 Par["lb"]*Par["nf"] 进行 reshape。
        self.unet = Unet2D(
            dim=unet_dim,
            Par=Par,
            dim_mults=dim_mults,
    ...
    def forward(self, x, time):
        # x: [B, lb, nf, nx, ny]
        # 先用 ConvLSTM 编码时间序列，保持通道数不变（hidden_dim = nf）
        clstm_out_list, _ = self.clstm(x)  # list 中最后一层输出：[B, lb, nf, nx, ny]
        x_enc = clstm_out_list[0]
        # 直接交给 Unet2D；其内部会归一化并按 [B, lb*nf, nx, ny] reshape
        return self.unet(x_enc, time)
